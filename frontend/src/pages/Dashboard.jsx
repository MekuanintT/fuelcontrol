...
export default function Dashboard({ API_BASE, token, role, vehicles, reloadVehicles, fuelRecords, reloadFuelRecords }) {
  ...
  const chartOptions = {
    ...
    scales: {
      x: { grid: { color: 'rgba(255,255,255,0.03)' }, ticks: { color: '#52525b', font: { family: 'Inter', size: 11 } } },
      y: { grid: { color: 'rgba(255,255,255,0.03)' }, ticks: { color: '#52525b', font: { family: 'Inter', size: 11 } } },
    },
  };
  ...
  const handleSaveVehicle = async (e) => {
    ...
    const payload = {
      ...
      fuel_quota: parseFloat(fuelQuota),
    ...
    try {
      ...
      resetForm(); await reloadVehicles(); await reloadFuelRecords();
  ...
  const resetForm = () => {
    setVehicleId(''); setPlateNumber(''); setOwnerName('');
    setFuelQuota(''); setWaitingHours(''); setStatusSelect('ACTIVE');
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', paddingBottom: 24, borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        <div>
          <h1 style={{ margin: '0 0 6px', fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 800, fontSize: 'clamp(1.75rem,3vw,2.25rem)', color: '#f4f4f5', letterSpacing: '-0.02em' }}>Fleet Dashboard</h1>
          <p style={{ margin: 0, color: '#71717a', fontSize: '0.875rem' }}>Real-time metrics, consumption analytics, and autonomous compliance.</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14 }}>
        {metrics.map(({ label, value, color, bg, border, icon: Icon }) => (
          <div key={label} className="card-metric" style={{ padding: 22, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <p style={{ margin: '0 0 6px', fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#52525b' }}>{label}</p>
              <p style={{ margin: 0, fontSize: '2rem', fontWeight: 800, fontFamily: 'Plus Jakarta Sans, sans-serif', color, letterSpacing: '-0.02em', lineHeight: 1 }}>{value}</p>
            </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div className="card-glass" style={{ padding: 24 }}>
            <SectionHeader icon={BarChart3} color="#a78bfa" title="Consumption by Vehicle">
              <span style={{ fontSize: 11, color: '#52525b' }}>Liters Distributed</span>
            </SectionHeader>
            <div style={{ height: 280 }}>
              {fuelRecords?.length ? <Bar data={getChartData()} options={chartOptions} /> : (
                <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#52525b', fontSize: '0.875rem' }}>No transaction history yet.</div>
              )}
            </div>
          </div>
              ...
              <table className="tbl">
                ...
                <tbody>
                  {vehicles?.length ? vehicles.map(v => (
                    <tr key={v.id}>
                      <td style={{ color: '#3f3f46', fontFamily: 'JetBrains Mono, monospace', fontSize: 12 }}>{v.id}</td>
                      <td><span className="plate-chip">{v.plate_number}</span></td>
                      <td style={{ color: '#e4e4e7', fontWeight: 500 }}>{v.owner_name}</td>
                      <td style={{ color: '#a1a1aa', fontWeight: 600 }}>{parseFloat(v.fuel_quota).toFixed(2)} L</td>
                      <td style={{ color: '#71717a' }}>{v.waiting_hours}h</td>
                      <td><span className={v.status==='ACTIVE' ? 'badge badge-active' : 'badge badge-blocked'}>{v.status}</span></td>
                      {role==='ADMIN' && (
                        <td style={{ textAlign: 'right' }}>
                          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                            <button onClick={() => startEdit(v)} style={{ display:'flex',alignItems:'center',gap:5,padding:'5px 12px',background:'rgba(139,92,246,0.08)',border:'1px solid rgba(139,92,246,0.2)',borderRadius:7,color:'#a78bfa',fontSize:12,cursor:'pointer',fontWeight:500 }}>
                              <Pencil size={12} /> Edit
                            </button>
                            <button onClick={() => handleDelete(v.id)} style={{ display:'flex',alignItems:'center',gap:5,padding:'5px 12px',background:'rgba(251,113,133,0.07)',border:'1px solid rgba(251,113,133,0.18)',borderRadius:7,color:'#fb7185',fontSize:12,cursor:'pointer',fontWeight:500 }}>
                    ...
                    <tr><td colSpan={role==='ADMIN'?7:6} style={{ textAlign:'center',padding:'32px 0',color:'#3f3f46' }}>No fleet vehicles registered.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div className="card-glass" style={{ padding: 22, borderLeft: '2px solid rgba(139,92,246,0.4)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
              <Sparkles size={16} color="#a78bfa" />
              <span style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700, fontSize: '0.9375rem', color: '#f4f4f5' }}>AI Insights</span>
            </div>
            <p style={{ margin: '0 0 16px', fontSize: '0.8125rem', color: '#71717a', lineHeight: 1.6 }}>
              FuelControl Copilot scans active vehicles and log patterns. Open the copilot to resolve warnings or simulate logs.
            </p>
          ...
          {role === 'ADMIN' && (
            <div ref={formRef} className="card-glass" style={{ padding: 22 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
                <PlusCircle size={16} color="#a78bfa" />
                <span style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700, fontSize: '0.9375rem', color: '#f4f4f5' }}>
                  {vehicleId ? 'Update Vehicle' : 'Register Vehicle'}
                </span>
              </div>
              <form onSubmit={handleSaveVehicle} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                {[
                  { id: 'plate', label: 'License Plate',    value: plateNumber,   onChange: e => setPlateNumber(e.target.value),   placeholder: 'AB-1234', type: 'text'   },
                  { id: 'owner', label: 'Owner Name',       value: ownerName,     onChange: e => setOwnerName(e.target.value),     placeholder: 'John Doe', type: 'text'  },
                  { id: 'quota', label: 'Fuel Quota (L)',   value: fuelQuota,     onChange: e => setFuelQuota(e.target.value),     placeholder: '100.00', type: 'number', step: '0.01' },
                  { id: 'hours', label: 'Cooldown (Hours)', value: waitingHours,  onChange: e => setWaitingHours(e.target.value),  placeholder: '24', type: 'number'      },
                ].map(f => (
                ...
                {vehicleId && (
                  <button type="button" onClick={resetForm} className="btn-ghost" style={{ borderRadius: 10, padding: '9px 16px', fontSize: '0.8125rem', textAlign: 'center' }}>
                    Cancel Edit
                  </button>
                )}
